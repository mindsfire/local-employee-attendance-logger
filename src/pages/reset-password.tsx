...
export default function ResetPassword() {
    ...
    }, [supabase.auth, router, isSuccess, debugInfo, isReady]);
    ...
    const onSubmit = (values: ResetPasswordFormValues) => {
        ...
        // Set up a listener for the auth state change that happens when password updates
        const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event: AuthChangeEvent, session: Session | null) => {
            console.log('[Reset Password] Auth event during password update:', event, 'Session exists:', !!session);

            // When password is updated successfully, Supabase fires a SIGNED_IN event
            if (event === 'SIGNED_IN' && session && !successHandled) {
                successHandled = true;
                subscription.unsubscribe();

                console.log('[Reset Password] Password update SUCCESS detected via auth state change!');
                ...
                console.log('[Reset Password] Success state has been set');

                // Update the password_changed flag in background
        ...
        // Timeout fallback - if no success after 15 seconds, show error
        const failureTimeout = setTimeout(() => {
            if (!successHandled) {
                console.error('[Reset Password] No success detected after 15 seconds');
                subscription.unsubscribe();
                setError('Password update is taking longer than expected. The password may have been changed - try logging in with your new password.');
                setIsLoading(false);
            }
        }, 15000);

        // Trigger the password update (fire and forget - we rely on the auth state change event)
        console.log('[Reset Password] Calling supabase.auth.updateUser...');
        supabase.auth.updateUser({
            password: values.password,
        }).then(({ error: updateError }: { error: { message: string } | null }) => {
            console.log('[Reset Password] updateUser promise finally resolved');
            if (updateError && !successHandled) {
                console.error('[Reset Password] Supabase update error from promise:', updateError);
            ...
            if (!successHandled) {
                console.error('[Reset Password] updateUser promise rejected:', err);
                clearTimeout(failureTimeout);
                subscription.unsubscribe();
